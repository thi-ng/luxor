  ...
  (defn infinity-curve
    ...
    ([w h d d1 d2 t smooth]
       (let [w2 (* w 0.5)
             ...
             d13 (* d d1)
             d23 (* d d2)
             nt (m/normalize (vec3 0 (- h2) (- d23 d13)))
             n0 (vec3 0 (- t) 0)
             n3 (vec3 0 0 (- t))
             n1 (-> (vec3 0 -1 0) (m/mix nt) (m/normalize t))
             n2 (-> (vec3 0 0 -1) (m/mix nt) (m/normalize t))
             pa (vec3 w1 0 0)   pa' (m/+ pa n0)
             pb (vec3 w2 0 0)   pb' (m/+ pb n0)
             pc (vec3 w2 0 d13) pc' (m/+ pc n0)
             pd (vec3 w1 0 d13) pd' (m/+ pd n0)
             pe (vec3 w1 0 d23) pe' (m/+ pe n1)
             pf (vec3 w2 0 d23) pf' (m/+ pf n1)
             pg (vec3 w1 h2 d)  pg' (m/+ pg n2)
             ph (vec3 w2 h2 d)  ph' (m/+ ph n2)
             pi (vec3 w1 h d)   pi' (m/+ pi n3)
             pj (vec3 w2 h d)   pj' (m/+ pj n3)]
