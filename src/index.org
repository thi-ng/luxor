...
#+BEGIN_SRC clojure :tangle ../babel/examples/test-scene.clj :mkdirp yes :padline no
  ...
  (do ;; wrap in `do` form to avoid full scene dump in repl (optional/cosmetic)
    (->
     ...
     (volume
      :inside {:type :clear :absorb (col/rgba 0.97 0.8 0.7) :abs-depth 0.2 :ior 2.04})

     ;; semi-translucent material w/ above medium as interior
     (material-matte-translucent
      :milk {:interior :inside :reflect (col/rgba 0.3 0.3 0.3) :transmit (col/rgba 0.8 0.7 0.6)})
     ;; standard matt white for background/room box
     (material-matte :white {:diffuse (col/rgba 0.8 0.8 0.8)})
...
|-----------+------------+-------------------------------------------+-----------------------------------+---------------------|
|     0.3.2 |        n/a | update dependencies, make geom compatible | =[thi.ng/luxor "0.3.2-SNAPSHOT"]= |                     |
