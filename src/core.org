...
#+BEGIN_SRC clojure :noweb-ref helpers
  ...
  (defn color? [x] (satisfies? col/IRGBConvert x))
  (defn optional-bool
    ...
    (let [eye (vec3 eye)
          ...
          opts {:fov [:float fov]
                ...
                :__lookat {:eye eye :target target
                           :up (if up
                                 (vec3 up)
                                 (gu/ortho-normal (m/- eye target) V3X))}}
          opts (cond
...
#+BEGIN_SRC clojure :noweb-ref volumes
  (defn volume
    [scene id {:keys [type ior absorb abs-scale abs-depth]
               :or {type      :clear
                    absorb    (col/rgba 1.0 1.0 1.0)
                    abs-scale 1.0
                    abs-depth 1.0
                    ior       :air}}]
    {:pre [(kw-or-str? id)
           ...
           (kw-or-num? ior)
           (optional keyword? presets/ior-presets ior)
           (color? absorb)
           (number? abs-scale)
           (number? abs-depth)]}
    (append
     ...
     {:__type     type
      ...
      :absorption [:log-color [@(col/as-rgba absorb) abs-scale abs-depth]]}))
...
#+BEGIN_SRC clojure :noweb-ref materials
  ...
  (defn material-glossy-coating
    [scene id {:keys [multi-bounce? specular ior roughness base]
               :or   {specular (col/rgba 0.8 0.8 0.8)
                      ior 0.0
                      roughness 0.075}
               :as   opts}]
    {:pre [(kw-or-str? id)
  ...
  (defn material-matte
    [scene id {:keys [diffuse sigma]
               :or   {diffuse (col/rgba 1.0 1.0 1.0) sigma 0}
               :as   opts}]
    {:pre [(kw-or-str? id)
           (color? diffuse)
           (number? sigma)]}
    (material-common
     scene id "matte"
     {:Kd    [:color @(col/as-rgba diffuse)]
      :sigma [:float sigma]}
     opts))

  (defn material-matte-translucent
    [scene id {:keys [reflect transmit sigma alpha conserve?]
               :or   {reflect   (col/rgba 0.3 0.3 0.3)
                      transmit  (col/rgba 0.65 0.65 0.65)
                      sigma     0
                      alpha     1.0
                      conserve? true}
               :as   opts}]
    ...
    (material-common
     ...
     {:Kr               [:color @(col/as-rgba reflect)]
      :Kt               [:color @(col/as-rgba transmit)]
      :sigma            [:float sigma]
      :energyconserving [:bool conserve?]}
     opts))

  (defn material-mirror
    [scene id {:keys [reflect film film-ior]
               :or   {reflect  (col/rgba 1 1 1)
                      film     0
                      film-ior 1.5}
               :as   opts}]
    {:pre [(kw-or-str? id)
...
#+BEGIN_SRC clojure :tangle ../babel/src/thi/ng/luxor/core.clj :noweb yes :mkdirp yes :padline no
  (ns thi.ng.luxor.core
    (:require
     ...
     [thi.ng.geom.vector :as v :refer [vec3 V3X]]
